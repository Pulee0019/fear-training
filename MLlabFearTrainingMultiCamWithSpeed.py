...
class OpenCVCamera:
    def __init__(self, root, arduino=None, camera_id=0, device_index=0, num_channels=1):
        ...
        self.frame_queues = [queue.Queue(maxsize=30) for _ in range(num_channels)]
    ...
    def _preview_loop(self):
        ...
        self.frame_num_counter0 = 0
        self.frame_num_counter = 0
        self.frame_num_counter1 = 0
        while self.previewing:
            ...
            if ret:
                if self.recording:
                    self.frame_num_counter0 += 1
                    if frame.shape[1] != self.width or frame.shape[0] != self.height:
                        frame = cv2.resize(frame, (self.width, self.height))
                    for i in range(self.num_channels):
                        if not self.frame_queues[i].full():
                            self.frame_num_counter += 1
                            self.frame_queues[i].put_nowait(frame.copy())
    ...
    def _record_loop(self, channel):
        while self.recording:
            try:
                ...
                time.sleep(0.01)
        print(self.frame_num_counter0)
        print(self.frame_num_counter)
        print(self.frame_num_counter1)
...
class FearTrainingGUI:
    ...
    def __init__(self, root):
        ...
        self.enable_cs_optogenetics = tk.BooleanVar(value=False)
        self.enable_us_optogenetics = tk.BooleanVar(value=False)
    ...
    def setup_ui(self):
        ...
        optogenetics_frame = ttk.LabelFrame(left_panel, text="Optogenetics Control")
        optogenetics_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Checkbutton(optogenetics_frame, text="Enable Optogenetics during CS", variable=self.enable_cs_optogenetics,
                       command=self.toggle_cs_optogentics).grid(row=0, column=0, padx=5, pady=5)

        ttk.Checkbutton(optogenetics_frame, text="Enable Optogenetics during US", variable=self.enable_us_optogenetics,
                       command=self.toggle_us_optogentics).grid(row=1, column=0, padx=5, pady=5)
        ...
        self.type_combo = ttk.Combobox(config_frame, textvariable=self.block_type, 
        ...
        self.sound_optogenetics_frame = ttk.Frame(config_frame)
        self.sound_shock_optogenetics_frame = ttk.Frame(config_frame)
        
        ttk.Label(self.sound_optogenetics_frame, text="Sound Duration (s):").grid(row=0, column=0, sticky="w")
        self.sound_duration_opt = tk.StringVar(value="5")
        ttk.Entry(self.sound_optogenetics_frame, textvariable=self.sound_duration_opt, width=14).grid(row=0, column=1)
        
        ttk.Label(self.sound_optogenetics_frame, text="Stimulation Location (s):").grid(row=1, column=0, sticky="w")
        self.stim_location_opt = tk.StringVar(value="0")
        ttk.Entry(self.sound_optogenetics_frame, textvariable=self.stim_location_opt, width=14).grid(row=1, column=1)
        
        ttk.Label(self.sound_shock_optogenetics_frame, text="Sound Duration (s):").grid(row=0, column=0, sticky="w")
        self.sound_duration_shock_opt = tk.StringVar(value="5")
        ttk.Entry(self.sound_shock_optogenetics_frame, textvariable=self.sound_duration_shock_opt, width=14).grid(row=0, column=1)
        
        ttk.Label(self.sound_shock_optogenetics_frame, text="Shock Duration (s):").grid(row=1, column=0, sticky="w")
        self.shock_lead_opt = tk.StringVar(value="2")
        ttk.Entry(self.sound_shock_optogenetics_frame, textvariable=self.shock_lead_opt, width=14).grid(row=1, column=1)
        
        ttk.Label(self.sound_shock_optogenetics_frame, text="Stimulation Location (s):").grid(row=2, column=0, sticky="w")
        self.stim_location_shock_opt = tk.StringVar(value="0")
        ttk.Entry(self.sound_shock_optogenetics_frame, textvariable=self.stim_location_shock_opt, width=14).grid(row=2, column=1)
        ...
        ttk.Button(device_frame, text="START", command=self.run_timeline, style="Accent.TButton", width=14).grid(row=4, column=0, padx=5, pady=5)
        ttk.Button(device_frame, text="EXIT", command=self.exit, style="Accent.TButton", width=14).grid(row=4, column=1, padx=5, pady=5)
    ...
    def on_type_change(self, event=None):
        ...
        if btype == "Sound+Shock":
            ...
            self.sound_optogenetics_frame.grid_remove()
            self.sound_shock_optogenetics_frame.grid_remove()
        elif btype == "Sound+Optogenetics":
            self.duration_frame.grid_remove()
            self.shock_frame.grid_remove()
            self.sound_optogenetics_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=2)
            self.sound_shock_optogenetics_frame.grid_remove()
        elif btype == "Sound+Shock+Optogenetics":
            self.duration_frame.grid_remove()
            self.shock_frame.grid_remove()
            self.sound_optogenetics_frame.grid_remove()
            self.sound_shock_optogenetics_frame.grid(row=2, column=0, columnspan=2, sticky="ew", pady=2)
        else:
            self.shock_frame.grid_remove()
            self.sound_optogenetics_frame.grid_remove()
            self.sound_shock_optogenetics_frame.grid_remove()
    ...
    def get_block_types(self):
        base_types = ["Wait", "Sound", "Sound+Shock"]
        if self.enable_cs_optogenetics.get():
            base_types.append("Sound+Optogenetics")
        if self.enable_us_optogenetics.get():
            base_types.append("Sound+Shock+Optogenetics")
        return base_types
    
    def toggle_cs_optogentics(self):
        self.type_combo['values'] = self.get_block_types()
        if self.block_type.get() not in self.get_block_types():
            self.block_type.set(self.get_block_types()[0])
        self.on_type_change()
        
    def toggle_us_optogentics(self):
        self.type_combo['values'] = self.get_block_types()
        if self.block_type.get() not in self.get_block_types():
            self.block_type.set(self.get_block_types()[0])
        self.on_type_change()
    ...
    def add_block(self):
        ...
        if btype == "Sound+Shock":
            ...
            try:
                sound_dur = float(self.sound_duration_opt.get())
                stim_location = float(self.stim_location_opt.get())
            except ValueError:
                messagebox.showerror("Error", "Please enter valid numbers for Sound+Optogenetics parameters")
                return
            if sound_dur <= 0:
                messagebox.showerror("Error", "Sound duration must be positive")
                return
            if stim_location < -30 or stim_location > 30:
                messagebox.showerror("Error", "Stimulation location must be between -30 and 30 seconds")
                return
            self.timeline.append((btype, sound_dur, stim_location))
            self.timeline_listbox.insert(tk.END, f"{btype} - Sound {sound_dur:.1f}s, Stim at {stim_location:.1f}s")
        elif btype == "Sound+Shock+Optogenetics":
            try:
                sound_dur = float(self.sound_duration_shock_opt.get())
                shock_lead = float(self.shock_lead_opt.get())
                stim_location = float(self.stim_location_shock_opt.get())
            except ValueError:
                messagebox.showerror("Error", "Please enter valid numbers for Sound+Shock+Optogenetics parameters")
                return
            if sound_dur <= 0 or shock_lead < 0:
                messagebox.showerror("Error", "Sound duration and shock duration must be positive, and shock start time must be non-negative")
                return
            if shock_lead > sound_dur:
                messagebox.showerror("Error", "Shock start time must be less than or equal to sound duration")
                return
            if stim_location < -28 or stim_location > 32:
                messagebox.showerror("Error", "Stimulation location must be between -28 and 32 seconds")
                return
            self.timeline.append((btype, sound_dur, shock_lead, stim_location))
            self.timeline_listbox.insert(tk.END, f"{btype} - Sound {sound_dur:.1f}s, Shock start {shock_lead:.1f}s before end, Stim at {stim_location:.1f}s")
        else:
            try:
    ...
    def _run_timeline(self):
        ...
        while i < len(self.timeline) and any(camera.recording for camera in self.cameras):
            ...
            if btype == "Wait":
                ...
                sound_dur = entry[1]
                stim_location = entry[2]
                
                stim_time = max(0, stim_location)
                ...
                if stim_time < sound_dur:
                    ...
                    laser_event_start = time.time()
                    self.trigger_laser()
                    laser_event_end = time.time()
                    
                    remaining_time = sound_dur - stim_time
                    if remaining_time > 0:
                ...
                self.event_log.append((event_start, event_end, 1))
                if stim_time < sound_dur:
                    self.event_log.append((laser_event_start, laser_event_end, 3))
            elif btype == "Sound+Shock+Optogenetics":
                sound_dur = entry[1]
                shock_lead = entry[2]
                stim_location = entry[3]
                shock_start = max(0, sound_dur - shock_lead)
                
                stim_time = max(0, shock_start + stim_location)
                ...
                if stim_time < sound_dur:
                    ...
                    if shock_start > stim_time:
                        time.sleep(shock_start - stim_time)
                else:
                    if shock_start > 0:
                        time.sleep(shock_start)
                ...
                remaining_time = sound_dur - max(stim_time, shock_start)
                if remaining_time > 0:
                    time.sleep(remaining_time)
    ...
    def trigger_stimulation(self, duration):
        try:
            task = Task()
            task.CreateAOVoltageChan("Dev2/ao0", "", -10.0, 10.0, 10348, None)
            sampling_rate = 5000
            ...
            reset_task = Task()
            reset_task.CreateAOVoltageChan("Dev2/ao0", "", -10.0, 10.0, 10348, None)
