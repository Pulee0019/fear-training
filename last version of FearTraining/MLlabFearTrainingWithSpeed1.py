...
class FLIRCamera:
    def __init__(self, root, arduino=None, camera_id=0):
        ...
        self.record_rate = 90.0
        self.fps = self.record_rate
        ...
        self.acq_queue = queue.Queue(maxsize=500)
        self.record_queue = queue.Queue(maxsize=500)
    ...
    def open(self):
        try:
            ...
            offset_x_node = PySpin.CIntegerPtr(self.nodemap.GetNode("OffsetX"))
            offset_y_node = PySpin.CIntegerPtr(self.nodemap.GetNode("OffsetY"))
            ...
            width_node = PySpin.CIntegerPtr(self.nodemap.GetNode("Width"))
            height_node = PySpin.CIntegerPtr(self.nodemap.GetNode("Height"))
            if PySpin.IsAvailable(width_node) and PySpin.IsWritable(width_node):
                width_node.SetValue(min(self.width, width_node.GetMax()))
                self.width = width_node.GetValue()
            if PySpin.IsAvailable(height_node) and PySpin.IsWritable(height_node):
                height_node.SetValue(min(self.height, height_node.GetMax()))
                self.height = height_node.GetValue()

            exposure_auto = PySpin.CEnumerationPtr(self.nodemap.GetNode("ExposureAuto"))
            ...
            exposure_time_node = PySpin.CFloatPtr(self.nodemap.GetNode("ExposureTime"))
            if PySpin.IsAvailable(exposure_time_node) and PySpin.IsWritable(exposure_time_node):
                exposure_time_node.SetValue(self.exposure_time)

            fps_enable = PySpin.CBooleanPtr(self.nodemap.GetNode("AcquisitionFrameRateEnable"))
            fps_value = PySpin.CFloatPtr(self.nodemap.GetNode("AcquisitionFrameRate"))
            if PySpin.IsAvailable(fps_enable) and PySpin.IsWritable(fps_enable):
                fps_enable.SetValue(True)
            if PySpin.IsAvailable(fps_value) and PySpin.IsWritable(fps_value):
                fps_value.SetValue(min(self.sample_rate, fps_value.GetMax()))
                self.fps = fps_value.GetValue()
                self.sample_rate = self.fps

            self.cam.BeginAcquisition()
            print(f"[INFO] Opened FLIR camera {self.camera_id} with resolution {self.width}x{self.height} @ {self.fps:.2f}fps")
        except Exception as e:
            raise RuntimeError(f"Failed to initialize FLIR camera: {e}")
    ...
    def close(self):
        ...
        try:
            ...
            print(f"[WARN] Error stopping FLIR acquisition: {e}")
        ...
        try:
            ...
            print(f"[INFO] FLIR camera {self.camera_id} closed")
        except Exception as e:
            print(f"[WARN] Error closing FLIR camera: {e}")
    ...
    def start_record(self, save_path):
        ...
        with self.recording_lock:
            if self.recording:
                print(f"[WARN] Camera {self.camera_id} is already recording")
                return None
            ...
            print(f"[DEBUG] FLIR start_record: {save_path}, Size=({self.width}, {self.height}), FPS={self.record_rate}")
        ...
        try:
            ...
            while self.previewing or self.recording:
                try:
                    ...
                    # print(f"[INFO] Got image: {image_data.shape}")
                    image_result.Release()
                    rgb_frame = cv2.cvtColor(image_data, cv2.COLOR_BAYER_BG2BGR)
                    ...
                    if self.previewing and (now - last_preview_time >= preview_interval):
                        ...
                        if not self.display_queue.full():
                            self.display_queue.put(rgb_frame.copy())
                    
                    if self.recording:
                        while self.acq_queue.qsize() > 100:
                            try:
                                self.acq_queue.get_nowait()
                            except queue.Empty:
                                break
                        self.acq_queue.put((rgb_frame.copy(), time.time()))

                except Exception as e:
                    print(f"[ERROR] FLIR acquisition failed: {e}")
                    break
                    
        except Exception as e:
            print(f"[ERROR] FLIR acquisition loop failed: {e}")
        finally:
            self.acquiring = False

    def _recording_loop(self):
        target_interval = 1.0 / self.record_rate
        ...
        while self.recording:
            ...
            if not self.acq_queue.empty():
                frame, _ = self.acq_queue.get()
                
                if frame.shape[:2] != (self.height, self.width):
                    print(f"[ERROR] Frame shape mismatch: {frame.shape[:2]} vs expected ({self.height}, {self.width})")
                    continue
        ...
        self.mark_ttl_off()
        self._flush_record_queue()

    def _flush_record_queue(self):
    ...
    def stop_record(self):
        with self.recording_lock:
            ...
            print(f"Recording stopped for camera {self.camera_id}, recorded {self.frame_counter} frames")
            return end_time
...
class OpenCVCamera:
    ...
    def start_record(self, save_path):
        ...
        if self.video_writer is None or not self.video_writer.isOpened():
            raise RuntimeError(
                f"VideoWriter failed. Path: {self.save_path}, "
...
class FearTrainingGUI:
    ...
    def add_camera(self):
        ...
        flir_frame.grid(row=3, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
        
        common_frame = ttk.Frame(popup)
        common_frame.grid(row=4, column=0, columnspan=2, padx=5, pady=5, sticky="ew")
        ...
        exposure_var = tk.DoubleVar(value=self.exposure_time.get())
        ttk.Entry(flir_frame, textvariable=exposure_var, width=10).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(flir_frame, text="Acquisition FPS:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
        sample_rate_var = tk.DoubleVar(value=self.sample_rate.get())
        ttk.Entry(flir_frame, textvariable=sample_rate_var, width=10).grid(row=1, column=1, padx=5, pady=5)
        ...
        def toggle_parameters():
            ...
            elif camera_type.get() == "OpenCVCamera":
        ...
        def create_camera():
            try:
                ...
                if camera_type.get() == "FLIRCamera":
                    camera = FLIRCamera(self.root, self.arduino, camera_id=camera_id)
