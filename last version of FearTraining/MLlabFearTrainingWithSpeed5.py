...
Edit on Mon Jul 21 18:46:00 2025
...
class HKCamera:
    def __init__(self, root, arduino=None, camera_id=0, device_index=0):
        ...
        self.running = False
        self.preview_callback = None
        self.save_paths = [None] * self.num_channels
        ...
        self.frame_queues = [queue.Queue(maxsize=2000) for _ in range(self.num_channels)]
        self.record_threads = [None] * self.num_channels
        ...
        while self.previewing:
            try:
                ...
                if ret == MV_OK:
                    ...
                    img_np = np.frombuffer(buf, dtype=np.uint8).reshape(
                        (stFrame.stFrameInfo.nHeight, stFrame.stFrameInfo.nWidth))
    ...
    def start_record(self, save_paths):
        ...
        for i in range(self.num_channels):
            self.record_threads[i] = threading.Thread(target=self._record_loop, args=(i,), daemon=True)
                    ...
                    if self.recording:
                        for i in range(self.num_channels):
                            if not self.frame_queues[i].full():
                                self.frame_queues[i].put((img_np.copy(), time.time()))

                    self.acqDuration.append(time.time() - t0)
                else:
                    print(f"[ERROR] Failed to get image buffer: {ret}. Retrying...")
                    time.sleep(0.005)
            except Exception as e:
                print(f"[ERROR] Acquisition loop error: {e}. Retrying...")
                time.sleep(0.005)
    ...
    def _record_loop(self, channel):
        ...
        while self.recording:
            ...
            try:
                img_np, timestamp = self.frame_queues[channel].get(timeout=0.5)
                # Convert to bytes for SDK input
                last_frame_data = img_np.tobytes()

                stInput.pData = cast(c_char_p(last_frame_data), POINTER(c_ubyte))
                stInput.nDataLen = len(last_frame_data)
                ret = self.cam.MV_CC_InputOneFrame(stInput)
                if ret != MV_OK:
                    print(f"[ERROR] Failed to input frame for channel {channel}: {ret}")
                frame_counter += 1
                ...
                target_interval = frame_counter * target_interval_t - (time.time() - start_time)
                if target_interval > 0:
                    time.sleep(target_interval)
                record_loop_end = time.time()
                self.recDuration.append(record_loop_end - record_loop_start)
            except queue.Empty:
                print(f"[WARN] Camera {self.camera_id} record queue empty")
                time.sleep(0.01)
    ...
    def stop_record(self):
        ...
        for i in range(self.num_channels):
            if self.record_threads[i] and self.record_threads[i].is_alive():
                self.record_threads[i].join(timeout=1.0)
...
class OpenCVCamera:
    ...
    def _record_loop(self, channel):
        while self.recording:
            try:
                frame = self.frame_queues[channel].get(timeout=0.5)
...
class FearTrainingGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(device_frame, text="START", command=self.run_timeline, 
                  style="Accent.TButton", width=14).grid(row=4, column=0, padx=5, pady=5)
        ttk.Button(device_frame, text="EXIT", command=self.exit, 
                  style="Accent.TButton", width=14).grid(row=4, column=1, padx=5, pady=5)
    ...
    def add_camera(self):
        ...
        device_combo = ttk.Combobox(device_select_frame, textvariable=device_var, width=40, state="readonly")
