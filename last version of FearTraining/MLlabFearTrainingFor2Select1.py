...
class OpenCVCamera:
    def __init__(self, root, arduino=None):
        ...
        self.gui_fps_var = tk.StringVar(value="FPS: --")
        self.width = 1280
        self.height = 720
        self.fps = 30.0
    ...
    def open(self, index):
        ...
        print(f"[INFO] Opened OpenCV camera with resolution {self.width}x{self.height} @ {self.fps:.2f}fps")
    ...
    def close(self):
        ...
        self.capture = None
        self.video_writer = None
        print("[INFO] OpenCV camera closed.")
    ...
    def _record_loop(self):
        ...
        while self.recording:
            ...
            if ret:
                ...
                sleep_time = max(0, target_frame_time - (process_time % target_frame_time))
...
class FearTrainingGUI:
    def __init__(self, root):
        ...
        # 创建两种相机对象
        self.hk_camera = HKCamera(root, arduino=self.arduino)
        self.cv_camera = OpenCVCamera(root, arduino=self.arduino)
        
        # 默认使用海康相机
        self.current_camera = self.hk_camera
        self.current_camera.gui_fps_var = tk.StringVar(value="FPS: --")
        ...
        self.camera_type = tk.StringVar(value="HKCamera")  # 相机类型选择
    ...
    def setup_ui(self):
        ...
        # 相机类型选择
        ttk.Label(control_frame, text="Camera Type:").grid(row=0, column=0, sticky="w", pady=2)
        camera_type_combo = ttk.Combobox(control_frame, textvariable=self.camera_type, 
                                       values=["HKCamera", "OpenCVCamera"], width=15)
        camera_type_combo.grid(row=0, column=1, sticky="ew", pady=2)
        camera_type_combo.bind("<<ComboboxSelected>>", self.on_camera_type_change)

        ttk.Label(control_frame, text="Block Type:").grid(row=1, column=0, sticky="w", pady=2)
        ...
        ttk.Label(control_frame, text="Filename Prefix:").grid(row=4, column=0, sticky="w", pady=(10, 2))
        ttk.Entry(control_frame, textvariable=self.user_filename_prefix).grid(row=4, column=1, sticky="ew", pady=(10, 2))
        ttk.Button(control_frame, text="Choose Save Directory", command=self.choose_directory).grid(row=5, column=0, columnspan=2, pady=5)
        ...
        self.fps_label = ttk.Label(status_frame, textvariable=self.current_camera.gui_fps_var)
    ...
    def on_camera_type_change(self, event=None):
        ...
        if camera_type == "HKCamera":
            self.current_camera = self.hk_camera
            print("Switched to HKCamera")
        elif camera_type == "OpenCVCamera":
            self.current_camera = self.cv_camera
            print("Switched to OpenCVCamera")
        self.current_camera.gui_fps_var = tk.StringVar(value="FPS: --")
        self.fps_label.config(textvariable=self.current_camera.gui_fps_var)
    ...
    def open_camera(self):
        ...
        # 对于OpenCV相机，需要选择设备
        if self.current_camera == self.cv_camera:
            ttk.Label(popup, text="Camera Device:").grid(row=4, column=0, padx=5, pady=5, sticky="e")
            camera_list = list_available_cameras()
            camera_var = tk.StringVar()
            camera_combo = ttk.Combobox(popup, textvariable=camera_var, values=camera_list, width=30)
            camera_combo.grid(row=4, column=1, padx=5, pady=5)
            if camera_list:
        ...
        def apply_params():
            try:
                ...
                # 根据当前相机类型设置参数
                if self.current_camera == self.hk_camera:
                    self.current_camera.set_parameters(
                        width=width,
                        height=height,
                ...
                elif self.current_camera == self.cv_camera:
                    self.current_camera.set_parameters(
                        width=width,
                        height=height,
                        exposure_time=exposure_var.get(),
                ...
                if self.current_camera == self.cv_camera:
                    ...
                    self.current_camera.open(camera_index)
                else:
    ...
    def toggle_preview(self):
        if not self.current_camera.previewing:
            try:
                self.current_camera.start_preview(self.update_image)
    ...
    def update_timer(self):
        if self.current_camera.recording:
    ...
    def _run_timeline(self):
        ...
        try:
            self.current_camera.start_record(save_path)
        except Exception as e:
        ...
        i = 0
        while i < len(self.timeline) and self.current_camera.recording:
        ...
        self.current_camera.stop_record()
        self.save_event_log()
    ...
    #     try:
    #         # 这里使用Arduino控制刺激，而不是NI设备
    #         if self.arduino and self.arduino.is_open:
    #             self.arduino.write(b'S')  # 发送刺激信号
    #             time.sleep(duration)
    #             self.arduino.write(b's')  # 停止刺激
    #             print(f"Stimulation delivered for {duration}s")
    #     except Exception as e:
