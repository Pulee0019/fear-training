...
class BaseAnalyzerApp:
        ...
        try:
            ...
            if n_groups == 1:
                ...
                for i in range(n_groups):
                    ax = self.fig.add_subplot(n_groups, 1, i+1)
                    axes.append(ax)
            
            group_colors = plt.cm.Set3(np.linspace(0, 1, n_groups))
            
            for group_idx, (group, group_animals) in enumerate(groups.items()):
                ax = axes[group_idx] if n_groups > 1 else axes[0]
                        ...
                        if channel_num in animal_data['channel_data']:
                            for wavelength, col_name in animal_data['channel_data'][channel_num].items():
                                if col_name and col_name in animal_data['fiber_cropped'].columns:
                                    animal_id = animal_data['animal_id']
                                    # Use different shades for animals within the same group
                                    color = base_color * (0.7 + 0.3 * animal_idx / max(1, len(group_animals)-1))
                ...
                ax.set_title(f"Raw Fiber Photometry Data - Group {group}")
                ...
                if group_animals and 'event_data' in group_animals[0]:
                    ...
                    event_time_absolute = group_animals[0].get('event_time_absolute', False)
                    ...
                    video_start = group_animals[0]['video_start_fiber']
            ...
            self.canvas.draw()
            self.set_status("Raw fiber data plotted with group visualization")
        except Exception as e:
    ...
    def smooth_data(self):
        ...
        try:
            ...
            # Apply smoothing and plot by group
            self.fig.clear()
            n_groups = len(groups)
            if n_groups == 1:
                ax = self.fig.add_subplot(111)
                        ...
                        if channel_num in animal_data['channel_data']:
                            ...
                            if target_col and target_col in animal_data['preprocessed_data'].columns:
                                ...
                                ax.plot(time_data, animal_data['preprocessed_data'][smoothed_col], 
                                       ...
                                       label=f'{animal_data["animal_id"]} CH{channel_num} smoothed')
                
                ax.set_title(f"Smoothed Data - Group {group} (window={window_size}, order={poly_order})")
                ax.set_xlabel("Time (s)")
                ax.set_ylabel("Signal Intensity")
                ax.grid(False)
                ax.legend()
    ...
    def baseline_correction(self):
        ...
        try:
            ...
            # Apply baseline correction and plot by group
            self.fig.clear()
            n_groups = len(groups)
            if n_groups == 1:
                ax = self.fig.add_subplot(111)
                    ...
                    for channel_num in animal_data['active_channels']:
                        if channel_num in animal_data['channel_data']:
                            ...
                            ax.plot(time_data, animal_data['preprocessed_data'][baseline_corrected_col], 
                                   color=color, linewidth=2, alpha=0.8,
                                   label=f'{animal_data["animal_id"]} CH{channel_num} baseline corrected')
                            
                            # Also plot the baseline
                            ax.plot(time_data, baseline_pred, 
                                   color=color, linestyle='--', alpha=0.5,
                                   label=f'{animal_data["animal_id"]} CH{channel_num} baseline fit')
                
                ax.set_title(f"Baseline Correction - Group {group} ({model_type} model)")
                ax.set_xlabel("Time (s)")
                ax.set_ylabel("Signal Intensity")
                ax.grid(False)
                ax.legend()
    ...
    def motion_correction(self):
        ...
        try:
            ...
            # Apply motion correction and plot by group
            self.fig.clear()
            n_groups = len(groups)
            if n_groups == 1:
                ax = self.fig.add_subplot(111)
                    ...
                    for channel_num in animal_data['active_channels']:
                        if channel_num in animal_data['channel_data']:
                            ...
                            ax.plot(time_data, animal_data['preprocessed_data'][motion_corrected_col], 
                                   color=color, linewidth=2, alpha=0.8,
                                   label=f'{animal_data["animal_id"]} CH{channel_num} motion corrected')
                
                ax.set_title(f"Motion Correction - Group {group}")
                ax.set_xlabel("Time (s)")
                ax.set_ylabel("Signal Intensity")
                ax.grid(False)
                ax.legend()
    ...
    def calculate_and_plot_dff(self):
        ...
        try:
            ...
            # Calculate dF/F and plot by group
            self.fig.clear()
            n_groups = len(groups)
            if n_groups == 1:
                ax = self.fig.add_subplot(111)
            ...
            for group_idx, (group, group_animals) in enumerate(groups.items()):
                ...
                ax.set_title(f"ΔF/F - Group {group}")
                ax.set_xlabel("Time (s)")
                ax.set_ylabel("ΔF/F")
                ax.grid(False)
                ax.legend()
    ...
    def calculate_and_plot_zscore(self):
        ...
        try:
            ...
            # Calculate Z-score and plot by group
            self.fig.clear()
            n_groups = len(groups)
            if n_groups == 1:
                ax = self.fig.add_subplot(111)
                        ...
                        if channel_num in animal_data['channel_data']:
                            ...
                            if baseline_std > 0:
                                ...
                                ax.plot(time_data, zscore_data, 
                                       color=color, linewidth=2, alpha=0.8,
                                       label=f'{animal_data["animal_id"]} CH{channel_num} Z-score')
                
                ax.set_title(f"Z-score - Group {group}")
                ax.set_xlabel("Time (s)")
