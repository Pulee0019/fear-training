...
def Batch(video_dict, bin_dict, mt_cutoff, FreezeThresh, MinDuration, SIGMA=1, accept_p_frames=False):
    ...
    Run FreezeAnalysis on folder of videos of specified filetype. More batch, defined by user.
    ...
    ftype = video_dict['ftype']

    video_files = []
    for root, dirs, files in os.walk(video_dict['dpath']):
        for file in files:
            if file.lower().endswith(f".{ftype}"):
                video_files.append(os.path.join(root, file))
    
    video_dict['FileNames'] = video_files
    
    summary_all = None
    
    for video_file in video_dict['FileNames']:
        file_name = os.path.basename(video_file)
        file_dir = os.path.dirname(video_file)
        
        video_dict['file'] = file_name
        video_dict['fpath'] = video_file
        
        rel_path = os.path.relpath(file_dir, video_dict['dpath'])
        if rel_path == '.':
            rel_path = ''
        
        print(f'Processing File: {os.path.join(rel_path, file_name)}')
        ...
        Motion = Measure_Motion(video_dict, mt_cutoff, SIGMA=SIGMA)  
        Freezing = Measure_Freezing(Motion, FreezeThresh, MinDuration)  
        SaveData(video_dict, Motion, Freezing, mt_cutoff, FreezeThresh, MinDuration)
        summary = Summarize(video_dict, Motion, Freezing, FreezeThresh,
                           MinDuration, mt_cutoff, bin_dict=bin_dict)
