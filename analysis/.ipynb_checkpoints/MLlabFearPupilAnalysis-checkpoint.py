...
class FreezingAnalyzerApp:
    def __init__(self, root):
        ...
        self.root.geometry("200x550")
        self.root.resizable(False, False)
        self.dlc_results_path = ""
        self.timestamp_path = ""
        self.ast2_path = ""
        self.events_path = ""  # New: Store events file path
        self.freezing_data = {}
        self.pupil_data = {}
        self.ast2_data = {}
        self.aligned_data = {}
        self.cam_id = None  # New: Store camera ID from DLC filename
        self.experiment_start = None  # New: Store experiment start time
        self.event_data = None  # New: Store event data
        
        main_frame = ttk.Frame(root, padding="20")
        main_frame.grid(row=0, column=0, sticky="NSEW")
        
        # Original buttons with new event button
        self.create_button(main_frame, "Load DLC Results", self.load_DLC_Results, 0)
        self.create_button(main_frame, "Load Timestamp CSV", self.load_timestamps, 1)
        self.create_button(main_frame, "Load Events CSV", self.load_events, 2)  # New
        self.create_button(main_frame, "Run Freezing Analysis", self.analyze, 3)
        self.create_button(main_frame, "Run Pupil Analysis", self.run_pupil_analysis, 4)
        
        # AST2 processing buttons
        self.create_button(main_frame, "Load AST2 File", self.load_ast2_file, 5)
        self.create_button(main_frame, "Align AST2 Data", self.align_ast2_data, 6)
        
        # Other buttons remain similar
        self.create_button(main_frame, "Export Results to CSV", self.export_csv, 7)
        self.create_button(main_frame, "Compare Raw vs Cleaned", self.plot_compare_raw_cleaning, 8)
        self.create_button(main_frame, "Plot Freezing Timeline", self.plot_freezing_windows, 9)
        self.create_button(main_frame, "Plot Pupil Distance", self.plot_pupil_distance, 10)
        self.create_button(main_frame, "Plot Combined Data", self.plot_combined_data, 11)  # Modified
    ...
    def load_DLC_Results(self):
        ...
        if self.dlc_results_path:
            ...
            if match:
                ...
                messagebox.showerror("Error", "Could not find camera ID in DLC filename (expected 'camX')")
                self.dlc_results_path = ""
    ...
    def load_events(self):
        ...
        if self.events_path:
            try:
                ...
                # Convert relative event times to absolute using experiment start
    ...
    # The following methods remain mostly unchanged but with updated comments
    def analyze(self):
    ...
    def compute_freezing(self, timestamps):
        ...
        threshold = np.percentile(speed_smooth, 30)
        ...
        start_time = timestamps['start_time'].iloc[0] if 'start_time' in timestamps.columns else 0
        end_time = timestamps['end_time'].iloc[-1] if 'end_time' in timestamps.columns else len(speed_smooth)/fps
    ...
    def plot_compare_raw_cleaning(self):
        ...
        if self.events_path and self.event_data is not None:
            for _, row in self.event_data.iterrows():
                color = "#b6b6b6ff" if row['Event Type'] == 0 else "#fcb500f0"
        ...
        plt.title("Speed Trace with Freezing Zones and Events")
        plt.legend()
        plt.tight_layout()
        plt.show()
    ...
    def plot_freezing_windows(self):
        ...
        x = df['time'].values
        # Convert to absolute time if possible
        if self.experiment_start is not None:
            x = self.experiment_start + x
    ...
    def export_csv(self):
        ...
        if not save_path:
            return
        # Add absolute time if available
        if self.experiment_start is not None:
            export_df = self.window_freeze.copy()
            export_df['absolute_time'] = self.experiment_start + export_df['time']
